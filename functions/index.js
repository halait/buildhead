...
exports.userExists = functions.https.onRequest((req, res) => {
	...
	admin.firestore().collection("users").where("username", "==", queryUsername).limit(1).get().then((snap) => {
		res.set("Access-Control-Allow-Origin", "*");
		if(snap.empty) res.send("");
		else res.send("1");
	});
});
...
exports.receiveMail = functions.https.onRequest((req, res) => {
	...
	Promise.all(promises)
		.then(() => {res.redirect("https://vigress.web.app/thank-you/");})
